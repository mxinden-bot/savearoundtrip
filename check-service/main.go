...
//
//	{ "domain", "alt_svc", "advertises_h3", "h3_handshake_ok", "error" }
//
...
func checkH3(ctx context.Context, host string) bool {
	tr := &http3.Transport{TLSClientConfig: &tls.Config{ServerName: host, MinVersion: tls.VersionTLS13}}
	defer tr.Close()
...
func handleCheck(w http.ResponseWriter, r *http.Request) {
	...
	ctx, cancel := context.WithTimeout(r.Context(), opTimeout+2*time.Second)
	defer cancel()

	res.AltSvc, res.AdvertisesH3 = checkAltSvc(ctx, domain)
	res.H3HandshakeOK = checkH3(ctx, domain)
